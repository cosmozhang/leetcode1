...
class Solution(object):
    def isAlienSorted(self, words, order):
        ...
        for i in range(len(words)-1):
            ...
            if not self.is_ordered(first_word, second_word, order):
                return False

        return True

    def is_ordered(self, f_w, s_w, order):
        ...
        while True:
            if len(f_w) < s+1:
                ...
                f_c = f_w[s]
                s_c = s_w[s]
                if order.index(f_c) == order.index(s_c):
                    s += 1
                elif order.index(f_c) < order.index(s_c):
                    flag = True
                    break
                else:
                    flag = False
